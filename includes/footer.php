 ...
 * Optional variables:
 *   $footer_scripts - string of extra inline JS to run after React loads (plain JS, not JSX — see the
 *                     text/babel note below if a caller ever genuinely needs JSX here)
...
<!-- React (UMD) — no more babel-standalone (2026-09-25): it was the single heaviest thing loaded on every
     signed-in page, compiling this same unchanging JSX in the browser on every page view for no reason.
     assets/js/shared/layout_react.js is the precompiled output; see its header comment before editing it. -->
...
<!-- Every current caller passes plain JS here (checked 2026-09-25) — no page actually needed JSX/Babel for this.
     If a future caller genuinely writes JSX into $footer_scripts, precompile it (see layout_react.js's header
     comment for the approach) rather than bringing babel-standalone back for one page. -->
<script>
  <?= $footer_scripts ?>
</script>
<?php endif; ?>
