    ...
    .then((data) => {
      ...
      data.forEach((p) => {
        const label =
          ...
          ")";
        policySelect.innerHTML +=
          '<option value="' + p.policy_id + '">' + label + "</option>";
      });
      // Auto-select: exact ID (POST roundtrip) OR exactly 1 policy
      if (forceSelectId) {
...
clientSearch.addEventListener("focus", function () {
  if (this.value.trim() && !clientIdInput.value)
    this.dispatchEvent(new Event("input"));
});

// Pre-fill from view_client.php shortcut (GET param)
if (
  typeof prefillClientId !== "undefined" &&
  prefillClientId &&
  prefillClientName
) {
  selectClient(prefillClientId, prefillClientName);
} else if (typeof savedClient !== "undefined" && savedClient) {
  // Re-select on validation fail (POST roundtrip) — restore saved policy
  loadPolicies(
    savedClient,
    typeof savedPolicy !== "undefined" && savedPolicy ? savedPolicy : null,
  );
}
